...
<div class="main">
	...
	<div id="section-rewards">
		<div class="title f-cb">
			...
			<a href="index.php" class="f-fr">返回首页</a>
		</div>
		<div class="des"><p><?php echo date("m月d日");?></p></div>
		<?php
		function hidtel($phone){
			...
			if($IsWhat == 1){
				...
				return  preg_replace('/(1[234578]{1}[0-9])[0-9]{4}([0-9]{4})/i','$1****$2',$phone);
			}
		}
		...
		$yesterday = $yesterday;
		$tmpimte =  $tmpimte - 1;
		$sql = "select * from wsdt where award in (2,4,6) and dtime > $yesterday and dtime < $tmpimte";
		$rs = mysql_query($sql);
		$list = array();
		...
		for($i=0;$i<count($list);$i++){
			if($list[$i]['award']=="2"){$yours = "中50元话费";}
			else if($list[$i]['award']=="4"){$yours = "中50元话费";}
			else if($list[$i]['award']=="6"){$yours = "中50元话费";}
			
			echo "<ul><li class=\"f-cb\"><div class=\"f-fl\">".$list[$i]['name']."</div><div style=\" margin-left:10px\" class=\"f-fl\">".$list[$i]['tel']."</div><div class=\"f-fr\">".$yours."</div><div class=\"f-fr\">".$list[$i]['award']."</div></li></ul>";
		}
	?>
