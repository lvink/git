...
// Configerations
// ------------------------------------
$plugin_votes_table	= 'plugin_sign_weisheng_dati';
...
//$section 			= leen_sql_escape(leen_getParameter('section'));
$section 			= 1;// 活动期数
...
// Start Validate
// ------------------------------------
$sql1 = "SELECT count(sid) as num from `{$plugin_votes_table}` WHERE `wxid`='{$wxid}' and signtime > $yesterday and signtime < $tmpimte";
...
$lastsigntime = $item['signtime'] ? $item['signtime'] : 0;
if($check_fromurl) {
...
if($_status) {
	...
	$sqlInsert = "INSERT INTO `{$plugin_votes_table}`" . 
		"(`username`,`phone`,`answers`,`score`,`seconds`,`section`,`signtime`,`signtimes`,`ip`,`status`,`wxid`) VALUES " . 
		"('{$username}','{$phone}','{$answers}','{$score}','{$seconds}','{$section}','{$signtime}','{$signtimes}','{$fromip}','{$status}','{$wxid}')";
	//$db->execute($sqlUpdate);
	$db->execute($sqlInsert);
	$sqlInsert = "INSERT INTO `plugin_sign_lvsenanjin` (`username`,`phone`,`answers`,`score`,`seconds`,`section`,`signtime`,`signtimes`,`ip`,`status`) VALUES ('砳砳','13100000000','10000','95','34','{$section}','{$signtime}','{$signtimes}','127.0.0.1','1')";
	//$db->execute($sqlInsert);
	//$db->execute($sqlInsert);
	//$db->execute($sqlInsert);
